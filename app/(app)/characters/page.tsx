...
export default function CharactersPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("characters")
      .select("id, name, role, physical_description, reference_image_path")
      .order("created_at");
    const rows: Character[] = data ?? [];
    await Promise.all(
      rows.map(async (c) => {
  ...
  async function uploadPhoto(character: Character, file: File) {
    ...
    else toast.success(`${character.name} now has a face`);
    setBusyId(null);
    void load();
  }
              ...
              <div className="min-w-0 flex-1">
                ...
                <label className="mt-3 inline-flex">
                  ...
                  <span className="inline-flex h-11 cursor-pointer items-center rounded-lg border border-border bg-background px-4 font-heading tracking-wide hover:bg-muted transition-colors">
                    ...
                    {c.reference_image_path ? "Replace Photo" : "Add Photo"}
                  </span>
                </label>
              </div>
            </div>
